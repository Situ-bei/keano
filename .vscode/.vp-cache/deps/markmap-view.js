...
function dsv_default(delimiter) {
  ...
  function parseRows(text, f) {
    var rows = [], N2 = text.length, I = 0, n = 0, t, eof = N2 <= 0, eol = false;
    ...
    function token() {
      ...
      if (text.charCodeAt(j2) === QUOTE) {
        while (I++ < N2 && text.charCodeAt(I) !== QUOTE || text.charCodeAt(++I) === QUOTE) ;
        if ((i = I) >= N2) eof = true;
...
function q(e) {
  ...
  function c3(s2, l, p) {
    const h = (u4, g) => {
      ...
      const m = p(u4) || [];
      return x3.children = m.length === 0 ? null : m.map((y3) => h(y3, x3)), x3.children && Object.assign(x3, x3.children.reduce(
        (y3, f) => ({
          height: Math.max(y3.height, f.height + 1),
          length: y3.length + f.length
        }),
        x3
      )), x3;
    };
  ...
  return Object.assign(i, {
    ...
    dump(s2) {
      const l = n("nodeSize"), p = (h) => (u4) => {
        const g = h + "  ", x3 = h + "    ", { x: m, y: y3 } = u4, f = l(u4), k2 = u4.children || [], v2 = k2.length === 0 ? " " : `,${g}children: [${x3}${k2.map(p(x3)).join(x3)}${g}],${h}`;
        return `{ size: [${f.join(", ")}],${g}x: ${m}, y: ${y3}${v2}},`;
      };
      return p(`
`)(s2);
    }
...
var le = (e, t, n) => {
  ...
  !a2 && s2 ? pe(e, t, s2, l) : a2 && !s2 && me(e, t, a2, c3);
};
var ce2 = (e, t) => {
...
`) })
    ...
    walkTree(t, (m, y3, f) => {
      var b, w, z;
      g += 1, m.children = (b = m.children) == null ? void 0 : b.map((E2) => ({ ...E2 })), n += 1;
      const k2 = O(
        j(
          "div",
          {
      ...
      v2 ? u4 += 1 : (u4 || c3 >= 0 && m.state.depth >= c3) && (m.payload = { ...m.payload, fold: 1 }), y3(), v2 && (u4 -= 1), g -= 1;
    });
    const x3 = Array.from(l.childNodes).map(
      (m) => m.firstChild
    );
    this._checkImages(l), x3.forEach((m) => {
      var y3;
      (y3 = m.parentNode) == null || y3.append(m.cloneNode(true));
    }), walkTree(t, (m, y3, f) => {
      var b;
      const k2 = m.state, v2 = k2.el.getBoundingClientRect();
      m.content = k2.el.innerHTML, k2.size = [
        Math.ceil(v2.width) + 1,
        Math.max(Math.ceil(v2.height), o)
      ], k2.key = [(b = f == null ? void 0 : f.state) == null ? void 0 : b.id, k2.id].filter(Boolean).join(".") + // FIXME: find a way to check content hash
      m.content, y3();
    }), l.remove(), p.remove();
  }
  _checkImages(t) {
  ...
  renderData(t) {
    ...
    s2(l);
    const p = l.descendants().reverse(), h = l.links(), u4 = linkHorizontal(), g = min(p, (r) => r.x - r.xSize / 2), x3 = max(p, (r) => r.x + r.xSize / 2), m = min(p, (r) => r.y), y3 = max(p, (r) => r.y + r.ySize - n);
    ...
    const f = t && p.find((r) => r.data === t) || l, k2 = f.data.state.x0 ?? f.x, v2 = f.data.state.y0 ?? f.y, b = this.g.selectAll(
      childSelector("g")
      ...
      "transform",
      (r) => `translate(${v2 + f.ySize - r.ySize},${k2 + f.xSize / 2 - r.xSize})`
    ...
    this.transition(it).attr("r", 6).attr("cx", (r) => r.ySize - n).attr("cy", (r) => r.xSize).attr("stroke", (r) => c3(r.data)).attr(
      ...
      (r) => {
        var d;
        return (d = r.data.payload) != null && d.fold && r.data.children ? c3(r.data) : "var(--markmap-circle-open-bg)";
      }
    );
  ...
  async fit(t = this.options.maxInitialScale) {
    const n = this.svg.node(), { width: i, height: o } = n.getBoundingClientRect(), { fitRatio: a2 } = this.options, { minX: c3, maxX: s2, minY: l, maxY: p } = this.state, h = p - l, u4 = s2 - c3, g = Math.min(
      i / h * a2,
      o / u4 * a2,
      t
    ), x3 = identity5.translate(
      (i - h * g) / 2 - l * g,
      (o - u4 * g) / 2 - c3 * g
    ).scale(g);
    return this.transition(this.svg).call(this.zoom.transform, x3).end().catch(noop);
  }
  findElement(t) {
    let n;
  ...
  async ensureView(t, n) {
    ...
    const o = this.svg.node(), { spacingHorizontal: a2 } = this.options, c3 = o.getBoundingClientRect(), s2 = transform(o), [l, p] = [
      ...
      bottom: 0,
      ...n
    }, x3 = [g.left - l, c3.width - g.right - p], m = [g.top - h, c3.height - g.bottom - u4], y3 = x3[0] * x3[1] > 0 ? V(x3, Math.abs) / s2.k : 0, f = m[0] * m[1] > 0 ? V(m, Math.abs) / s2.k : 0;
    if (y3 || f) {
      const v2 = s2.translate(y3, f);

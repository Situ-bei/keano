      ...
      <div class="link-card">
        <a class="card-body" :href="item.url" target="_blank">
          <img class="link-picture" 
            :src="`https://s0.wp.com/mshots/v1/${encodeURIComponent(item.url)}?w=323&h=200`" 
            alt="logo"
...
import { collectData, type CollectOptions, VueEcology  } from "../data/collectData";
...
let linkDatas: CollectOptions[];
switch (props.type) {
  case "collect":
    linkDatas = collectData;
    break;
  case "books":
    linkDatas = collectData;
    break;
  case "VueEcology":
  linkDatas = VueEcology;
  break;
  default:
    linkDatas = [];
    break;
